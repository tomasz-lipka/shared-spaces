...
class TestDeleteMember(TestCase):
    ...
    def test_del_other_member_as_not_admin(self):
        ...
        response = self.client.delete(
            f"/spaces/{space_id}/members/{admin.get('id')}", headers={"Authorization": f"Bearer {member_token}"})
    ...
    def test_del_myself_as_member(self):
        ...
        add_member(self.client, space_id, member.get('login'), admin_token)
        response = self.client.delete(
            f'/spaces/{space_id}/members/{member.get("id")}', headers={"Authorization": f"Bearer {member_token}"})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data, b'Member deleted')
